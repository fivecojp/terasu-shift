# TERASU-Shift 実装プロンプト（Cursor用）v3
...
JST の今日日付が必要な場合は `Intl.DateTimeFormat('sv-SE', { timeZone: 'Asia/Tokyo' })` を使う。
...
既存テーブルは RLS ポリシーなし・anon から読めない。新規テーブルも RLS が有効なため、
`createServerClient` や `createBrowserClient` を Server Actions で使うと：
- データが空で返る（SELECT）
- INSERT/UPDATE が「row-level security policy」エラーで失敗する

**`createServiceClient()` を使う箇所:**
...
- **デフォルト表示期間**: `?ym` なしでアクセス時、`page.tsx` サーバー側で
  `resolveDefaultRequestMonthAndPeriod(settings, todayYmd)` を呼び、
  締切が切れていない最も近い期間の月に `redirect('/request?ym=YYYY-MM')` する

### シフト作成（/schedule）PC/タブレット・スマホ対応
- 月移動 / 表示期間切替 / 希望↔シフトトグル（leaderのみ）
- 公開ステータス + 「公開する」ボタン
  - PC（lg以上）: ヘッダー内に表示
  - スマホ（lg未満）: ☰ドロワー内・leader向けブロック先頭
- CSVエクスポート / 未提出スタッフ警告
- general: 閲覧専用（ガント・公開・CSV・設定・セルクリック非表示）
...
- JST 今日日付の取得: `Intl.DateTimeFormat('sv-SE', { timeZone: 'Asia/Tokyo' }).format(new Date()).slice(0, 10)`
