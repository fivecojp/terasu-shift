...
最終更新: 2026-04-29
バージョン: v2.0（UI改修・レスポンシブ対応・ナビゲーション整理後）
...
- 今日の日付列: `bg-slate-700 text-white font-bold`でハイライト、マウント時・表示幅変更時に `scrollIntoView` で自動スクロール
- ガントチャート: 時間軸（正時ラベル）＋縦グリッド線付き
- 表示幅: 1週間・2週間・半月・1か月
...
### 共通コンポーネントスタイル
```
プライマリボタン: bg-slate-700 text-white hover:bg-slate-800 rounded-lg
セカンダリボタン: border border-zinc-300 text-zinc-600 hover:bg-zinc-50 rounded-lg
input/select:    border border-zinc-300 rounded-lg focus:ring-2 focus:ring-slate-400
...
| /schedule グリッド幅レスポンシブ | 🔄 対応中 | テーブル幅がウィンドウ幅に追従しない問題 |
| /schedule 今日列自動スクロール | 🔄 対応中 | 1か月表示で今日が見えないケースあり |
